...
export function AiMentorChat({
  ...
  useEffect(() => {
    try {
      localStorage.setItem('cgl_ai_chat_expanded', String(isExpanded));
    } catch {}
  }, [isExpanded]);
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`fixed z-[60] flex flex-col bg-white overflow-hidden transition-all duration-200 ${
              isExpanded
                ? 'inset-0 w-full h-full rounded-none border-0 shadow-none'
                : 'bottom-4 right-4 w-[92vw] sm:w-[440px] h-[580px] max-h-[85vh] rounded-2xl border border-slate-200/90 shadow-2xl'
            }`}
          >
            {/* Header */}
            <div className="px-4 py-3 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white flex items-center justify-between shadow-md select-none shrink-0">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {isExpanded && (
                      <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-white/10 text-indigo-200 border border-white/10 uppercase tracking-wider hidden sm:inline-block">
                        Full Screen
                      </span>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg text-slate-300 hover:text-white hover:bg-white/10 transition-colors inline-flex cursor-pointer"
                  title={isExpanded ? 'Restore window (Exit Full Screen)' : 'Full screen'}
                >
            ...
            <AnimatePresence>
              {showContextDrawer && (
                <motion.div
                  ...
                  className="bg-indigo-50/90 border-b border-indigo-100 px-3.5 sm:px-6 py-2.5 text-[11px] text-slate-700 overflow-hidden shrink-0"
                >
