...
export const SillyMistakesAggregateView: React.FC<SillyMistakesAggregateViewProps> = ({
  ...
  const getCleanQuestionPreview = (raw: string) => {
    if (!raw) return '';
    return raw
      .replace(/\\(?:text|textbf|textit|mathrm|frac|sqrt|left|right)\b/g, '')
      .replace(/[{}\$\\_]/g, ' ')
      .replace(/\s+/g, ' ')
      .trim();
  };

  const toggleExpand = (qKey: string) => {
  ...
  // Handle Ask Tommy AI for a specific question
  const handleAskAiForQuestion = (q: Question, idx: number) => {
    const sillyNote = (q as any).sillyNote || (q as any).sillyMistakeNote || '';
    window.dispatchEvent(new CustomEvent('cgl_ask_ai_question', {
      detail: {
        questionNumber: idx + 1,
        questionText: q.question,
        options: q.options,
        userAnswer: (q as any).userAnswer || (q as any).chosenOption,
        correctAnswer: q.answer,
        solution: q.solution,
        topic: q.topic,
        sourceType: (q as any).sourceType || 'full_mock',
        sourceLabel: sillyNote ? `Silly Mistake: ${sillyNote}` : 'Silly Mistake Log',
        testName: (q as any).testName,
        rcaMode: 'silly',
        rcaTag: 'S',
        rcaTagName: 'Silly Mistake',
        sillyMistakeNote: sillyNote
      }
    }));
  };

  // Handle Ask Tommy AI for aggregate pattern audit
  const handleAskAiGlobalAudit = () => {
    const topMistakes = aggregatedReasonTags.slice(0, 5).map(t => `${t.tag} (${t.count}x)`).join(', ');
    const prompt = `Tommy, please analyze my Silly Mistake patterns in SSC CGL preparation for ${activeSubject === 'all' ? 'All Subjects' : activeSubject}.\n\nTotal Silly Mistakes Recorded: ${displayedQuestions.length}\nTop Recurring Mistake Traps: ${topMistakes || 'Various calculation and reading slips'}\n\n1. What are the common psychological or time-pressure triggers causing these slips?\n2. What strict 5-second verification protocol should I follow before marking an answer in the exam?\n3. How can I eliminate calculation errors in topics like ${displayedQuestions.slice(0, 3).map(q => q.topic).filter(Boolean).join(', ')}?`;
    ...
    window.dispatchEvent(new CustomEvent('cgl_ask_ai_question', {
      detail: {
        ...
        correctAnswer: '',
        solution: '',
        topic: 'Silly Mistake Remediation',
        sourceType: 'full_mock',
        sourceLabel: `Aggregate Silly Mistakes Audit (${displayedQuestions.length} Slips)`
      }
    }));
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-4 border-b border-slate-100">
          ...
          <div className="flex items-center gap-2 self-start md:self-auto flex-wrap">
            <button
              ...
              onClick={() => {
                const allOpen = displayedQuestions.length > 0 && displayedQuestions.every(q => expandedQIds.has(String(q.id || q.question)));
                if (allOpen) {
                  setExpandedQIds(new Set());
                } else {
                  setExpandedQIds(new Set(displayedQuestions.map(q => String(q.id || q.question))));
                }
              }}
              ...
              title="Toggle expand or collapse all questions"
            >
              {displayedQuestions.length > 0 && displayedQuestions.every(q => expandedQIds.has(String(q.id || q.question))) ? (
                <>
            ...
            <button
              type="button"
              disabled={displayedQuestions.length === 0}
              onClick={handleAskAiGlobalAudit}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-purple-700 bg-purple-50 hover:bg-purple-100 border border-purple-200/90 rounded-xl shadow-xs transition cursor-pointer active:scale-95 disabled:opacity-40"
              title="Get an AI comprehensive diagnostic analysis of your recurring silly mistakes"
            >
      ...
      <div className="space-y-3">
        {displayedQuestions.length === 0 ? (
          ...
          displayedQuestions.map((q, idx) => {
            ...
            const isExpanded = expandedQIds.has(qKey);
            ...
            return (
              ...
              >
                ...
                <div className="bg-slate-50/80 px-4 py-2.5 border-b border-slate-100 flex flex-wrap items-center justify-between gap-2">
                  ...
                  {/* PROMINENT SILLY MISTAKE RECORDED TAG & AI CHIP */}
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <button
                      type="button"
                      onClick={() => handleAskAiForQuestion(q, idx)}
                      className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold text-purple-700 bg-purple-50 hover:bg-purple-100 border border-purple-200 shadow-2xs transition cursor-pointer active:scale-95"
                      title="Ask Tommy AI about this silly mistake"
                    >
                ...
                {/* Silly Mistake Summary Body (Clean, Compact Default) */}
                <div className="p-3 sm:p-4 bg-white">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 bg-rose-50/50 rounded-xl border border-rose-100 hover:bg-rose-50/80 transition">
                    <div className="flex items-start gap-2.5 min-w-0 flex-1">
                      <span className="text-base shrink-0 mt-0.5">⚠️</span>
                      <div className="min-w-0 flex-1 space-y-0.5">
                        <div className="text-xs font-bold text-rose-950 flex items-center gap-1.5 flex-wrap">
                          <span className="text-rose-600 font-black uppercase text-[10px] tracking-wider">Mistake:</span>
                          <span className="bg-white/80 px-2 py-0.5 rounded-md border border-rose-200/60 shadow-2xs font-bold">
                            {q.sillyNote}
                          </span>
                        </div>
                        <div className="text-[11px] text-slate-500 font-medium truncate" title={q.question}>
                          <span className="font-semibold text-slate-700">Question: </span>
                          {getCleanQuestionPreview(q.question)}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0 self-end sm:self-center flex-wrap">
                      <button
                        type="button"
                        onClick={() => handleAskAiForQuestion(q, idx)}
                        className="h-7 px-2.5 text-xs font-bold text-purple-700 bg-purple-50 hover:bg-purple-100 hover:border-purple-300 rounded-lg border border-purple-200 transition flex items-center gap-1.5 cursor-pointer shadow-2xs active:scale-95"
                        title="Ask Tommy AI to analyze this silly mistake"
                      >
                        <Sparkles className="w-3 h-3 text-purple-600" />
                        <span>Ask AI</span>
                      </button>
                      ...
                      <button
                        ...
                        className={`h-7 px-3 text-xs font-bold rounded-lg border transition flex items-center gap-1.5 cursor-pointer shadow-2xs ${
                          isExpanded
                            ? 'bg-indigo-600 text-white border-indigo-600'
                            : 'bg-white text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 border-indigo-200'
                        }`}
                      >
                      ...
                      <button
                        ...
                        className="h-7 px-3 text-[11px] font-bold text-white bg-slate-900 hover:bg-slate-800 rounded-lg shadow-2xs transition flex items-center gap-1 cursor-pointer active:scale-95"
                  ...
                  {/* Expanded Question Details (Revealed Only on Demand) */}
                  {isExpanded && (
                    <div className="mt-3 pt-3 border-t border-slate-100 space-y-3">
                      ...
                      <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => toggleExpand(qKey)}
                          className="h-7 px-2.5 text-[11px] font-bold text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded-lg transition flex items-center gap-1 cursor-pointer"
                        >
