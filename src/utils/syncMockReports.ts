 ...
 * - Inserts any new bundled mocks that are not yet in storage
 */
export function syncMockReports(
  ...
  for (const item of normalizedSaved) {
    ...
    const match = bundledList.find(b => {
  ...
  // Include any bundled mocks not present in saved
  for (const b of bundledList) {
    if (!matchedBundledIds.has(b.id)) {
      result.push(b);
